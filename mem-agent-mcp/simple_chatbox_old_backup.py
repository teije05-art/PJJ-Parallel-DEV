...
# REPO_ROOT is already the mem-agent-mcp directory, no need to append it again
...
class ApprovalRequest(BaseModel):
    """Request for user approval (proposal or checkpoint)"""
    ...
    frameworks_evolved: Optional[int] = None
...
class ApprovalResponse(BaseModel):
    ...
    approval_id: str
    approval_status: str  # "approved", "rejected", "adjust_requested", "paused"
    adjusted_proposal: Optional[str] = None  # If adjust_requested
    adjusted_strategy: Optional[str] = None  # If adjust_requested for checkpoint
...
async def start_planning(request: PlanRequest):
    ...
    # Store current goal in session (needed for approval gate continuation)
    session["current_goal"] = goal_to_plan
    session["current_max_iterations"] = request.max_iterations
    session["current_checkpoint_interval"] = request.checkpoint_interval
    ...
    try:
        ...
        if request.max_iterations > 1:
            ...
            if not hasattr(request, 'proposal') or not request.proposal:
                ...
                # CRITICAL FIX: Pass max_iterations and checkpoint_interval to proposal request
                # This ensures user's selections are used instead of defaults
                proposal_request = ProposalRequest(
                    goal=goal_to_plan,
                    session_id=session_id,
                    max_iterations=request.max_iterations,  # Use user's selected value
                    checkpoint_interval=request.checkpoint_interval  # Use user's selected value
                )
            ...
            # APPROVAL GATE: Return ApprovalRequest instead of auto-approving
            ...
            if "pending_approvals" not in session:
            ...
            # Return the approval request to user
            print(f"✅ Proposal generated. Awaiting user approval...")
            return PlanResponse(
                status="awaiting_approval",
                iterations=0,
                results={
                    "approval_request": approval_request.dict(),
                    "next_action": f"POST to /api/approve with approval_id={approval_id} and approval_status='approved'|'rejected'|'adjust_requested'"
                },
                session_id=session_id,
                timestamp=datetime.now().isoformat(),
                execution_time=time.time() - start_time,
...
def _run_multi_iteration_planning_with_memagent(orchestrator, goal: str, proposal: str,
    ...
    try:
        ...
        llama_planner = LlamaPlanner(orchestrator.agent, orchestrator.memory_path)
...
class ProposalRequest(BaseModel):
    ...
    goal: str
    selected_entities: List[str] = []  # Entities to search (defaults to all if empty)
    selected_agents: List[str] = []  # Agents to use: planner, verifier, executor, generator
    max_iterations: int = 6  # How many iterations to plan for
    checkpoint_interval: int = 3  # When to show checkpoints
    session_id: Optional[str] = None
...
"""
        ...
        if 'planner' in selected_agents or True:  # Always call planner
            try:
                ...
                planning_agent = PlannerAgent(agent, Path(planner.memory_path))
        ...
        if 'verifier' in selected_agents:
            try:
                ...
                verifier_agent = VerifierAgent(agent, Path(planner.memory_path))
        ...
        if 'executor' in selected_agents:
            try:
                ...
                executor_agent = ExecutorAgent(agent, Path(planner.memory_path))
...
class ApprovalResponseSubmission(BaseModel):
    """Request model for submitting approval response."""
    approach: dict
...
async def submit_approval(request: ApprovalResponse):
    """
    Handle user approval/rejection of proposal or checkpoint.

    For proposals:
    - approved: Start multi-iteration planning
    - rejected: Cancel planning
    - adjust_requested: Regenerate proposal with feedback

    For checkpoints:
    - approved: Continue iterations
    - paused: Skip to final synthesis
    - adjust_requested: Modify strategy for next iterations
    """
    # Extract session_id from approval_id (format: "proposal_sessionXXX" or "checkpoint_iter3_sessionXXX")
    parts = request.approval_id.split("_")
    if len(parts) < 2:
        return {"status": "error", "message": "Invalid approval_id format"}

    # Find session_id (usually last component)
    session_id = None
    for part in reversed(parts):
        if part.startswith("session") or len(part) == 36:  # UUID format
            session_id = part
            break

    if not session_id:
        return {"status": "error", "message": "Could not extract session_id"}

    session_id, session = get_or_create_session(session_id)

    # Verify approval exists in session
    if "pending_approvals" not in session:
        session["pending_approvals"] = {}

    if request.approval_id not in session["pending_approvals"]:
        return {
            "status": "error",
            "message": f"Approval {request.approval_id} not found in session"
        }

    approval = session["pending_approvals"][request.approval_id]
    ...
    if request.approval_status == "rejected":
        ...
        return {
            "status": "cancelled",
            "reason": f"user_rejected_{approval_type}",
            "message": f"Planning cancelled. {approval_type.title()} was rejected.",
            "session_id": session_id
        }

    if request.approval_status == "paused":
        # User requested pause at checkpoint
        del session["pending_approvals"][request.approval_id]
        session["checkpoint_paused"] = True
        session["paused_at_iteration"] = approval.get("request", {}).get("iteration")
        return {
            "status": "paused",
            "message": "Checkpoint paused. Proceeding to final synthesis with current iterations.",
            "session_id": session_id,
            "paused_at_iteration": session["paused_at_iteration"]
        }
    ...
    if request.approval_status == "adjust_requested":
        ...
        if approval_type == "proposal" and request.adjusted_proposal:
            session["adjusted_proposal"] = request.adjusted_proposal
            session["proposal_adjustment_feedback"] = request.feedback
            return {
                "status": "adjusting",
                "message": "Proposal adjustment received. Regenerating proposal...",
                "session_id": session_id
            }

        elif approval_type == "checkpoint" and request.adjusted_strategy:
            session["checkpoint_strategy_adjustment"] = request.adjusted_strategy
            session["checkpoint_adjustment_feedback"] = request.feedback
            return {
                "status": "adjusting_strategy",
                "message": "Strategy adjustment received. Continuing with modified approach...",
                "session_id": session_id
            }

    if request.approval_status == "approved":
        # User approved
        approval_data = session["pending_approvals"][request.approval_id]
        del session["pending_approvals"][request.approval_id]

        if approval_type == "proposal":
            # START MULTI-ITERATION PLANNING
            proposal = approval_data.get("proposal", "")
            max_iterations = approval_data.get("max_iterations", 6)
            checkpoint_interval = approval_data.get("checkpoint_interval", 3)
            goal = session.get("current_goal", "Unknown goal")

            # Get orchestrator and start planning
            orchestrator = get_or_create_orchestrator(session_id)
            ...
            try:
                ...
                return PlanResponse(
                    status="error",
                    iterations=0,
                    results={"error": f"Planning failed: {str(e)}"},
                    session_id=session_id,
                    timestamp=datetime.now().isoformat(),
                    execution_time=0
                )

        elif approval_type == "checkpoint":
            # Continue iterations
            session["checkpoint_approved"] = True
            return {
                "status": "continuing",
                "message": "Checkpoint approved. Continuing iterations...",
                "session_id": session_id
            }

    return {
        "status": "error",
        "message": f"Unknown approval status: {request.approval_status}"
    }
...
async def execute_plan(request: ExecutePlanRequest):
    ...
    if request.max_iterations and request.max_iterations > 1:
        ...
        # Try to import and use LlamaPlanner if available
        if LLAMA_PLANNER_AVAILABLE:
            try:
                llama_planner = LlamaPlanner(orchestrator.agent, orchestrator.memory_path)
            except Exception as e:
                print(f"⚠️ Warning: LlamaPlanner initialization failed: {e}")
                print(f"Continuing with orchestrator-only multi-iteration planning...")
        else:
            print(f"⚠️ LlamaPlanner not available, using orchestrator-only planning")
        ...
        # CRITICAL FIX: Save the final comprehensive multi-iteration plan to MemAgent (ONLY ONCE, at the end)
        # This ensures we have ONE complete final plan instead of multiple per-iteration entities
        if results.get('success') and results.get('combined_plan'):
            try:
                planner = LlamaPlanner(orchestrator.agent, orchestrator.memory_path)

                # Create metadata for the final comprehensive plan
                final_metadata = {
                    'iterations_completed': results.get('completed_iterations', 0),
                    'max_iterations': results.get('total_iterations', 0),
                    'unique_frameworks': results.get('unique_frameworks', 0),
                    'total_data_points': results.get('total_data_points', 0),
                    'total_insights': results.get('total_insights', 0),
                    'checkpoint_count': len(results.get('checkpoints', [])),
                    'synthesis_type': 'comprehensive_multi_iteration'
                }
                ...
                save_result = planner.save_plan(
                    goal=request.goal,
                    plan_content=results.get('combined_plan', ''),
                    execution_metadata=final_metadata
                )

                print(f"✅ Final comprehensive plan saved to memory")
                print(f"   • Plan ID: {save_result.get('plan_id')}")
                print(f"   • Location: /local-memory/plans/")

            except Exception as e:
                print(f"⚠️ Warning: Could not save final plan: {e}")
    ...
    try:
        # Check if LlamaPlanner is available
        if not LLAMA_PLANNER_AVAILABLE:
            return PlanResponse(
                status="error",
                iterations=0,
                results={"error": "LlamaPlanner not available"},
                session_id=session_id,
                timestamp=datetime.now().isoformat(),
                execution_time=0
            )
    ...
    <div class="container">
        ...
        <div id="approval-gate-modal" class="modal" style="display: none;">
            <div class="modal-content">
                ...
                <div class="approval-section">
                    <h3>⚙️ Planning Configuration</h3>
...
${data.plan_content || '⚠️ No plan content generated.'}
...
`;
                    ...
                    try {
                        const saveResponse = await fetch('/api/save-entity', {
                            ...
                            body: JSON.stringify({
                                goal: pendingExecution.goal,
                                plan_content: data.plan_content || 'Plan executed but no content generated',
                                session_id: sessionId
                            })
                        });
